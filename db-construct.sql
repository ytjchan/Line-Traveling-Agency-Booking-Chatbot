...
create table TourOffering (TourID varchar(5) references Tour(TourID), OfferID varchar(8) constraint check_id_length check(length(OfferID)=8), Date date, TourGuideLineID varchar(40) references TourGuide(LineID) not null, Hotel varchar(50), Price smallint constraint check_price check (Price>0), MaxCapacity smallint constraint check_capacity check (MaxCapacity>0 and MaxCapacity>MinRequirement), MinRequirement smallint constraint check_requirement check (MinRequirement>0), Confirmed boolean, primary key(TourID, OfferID));
...
create table Booking (BookingID varchar(20) primary key, LineID varchar(40) references Booker(LineID) not null, TourID varchar(5) not null, OfferID varchar(8) not null, Adults smallint, Children smallint, Toddlers smallint, AmountPaid real, SpecialRequest varchar(500), Cancelled boolean default false, foreign key (TourID, OfferID) references TourOffering(TourID, OfferID));
create table Question (QID integer primary key, LineID varchar(40) references Booker(LineID) not null, FullQuestion varchar(200) not null, LastFiveQuestion varchar(1000), Resolved boolean default false);
...
insert into touroffering values ('1A001', '00000001', '2017-1-1', 'jeffer', 'UST Hall 5', 500, 20, 4);
insert into booker values ('12345678', 'Jeffer Chan', 'A123456', 98765432, 20);
insert into booking values ('1234567890', '12345678', '1A001', '00000001', 2, 0, 0, 100, 'No smoking room');
