...
create table Booking (BookingID varchar(20) primary key, LineID varchar(40) references Booker(LineID) not null, TourID varchar(5) not null, OfferID varchar(8) not null, Adults smallint, Children smallint, Toddlers smallint, AmountPaid real, SpecialRequest varchar(100), Cancelled boolean default false, foreign key (TourID, OfferID) references TourOffering(TourID, OfferID));
create table Question (QID integer primary key, LineID varchar(40) references Booker(LineID) not null, FullQuestion varchar(100) not null, LastFiveQuestion varchar(500), Resolved boolean default false);
